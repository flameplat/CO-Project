...
GetLowerDigit Macro
              AND al, 0Fh
              ENDM
makeSubBytes Macro PlainText
        Local oneSubByte
             push cx 
             MOV cx, 16
             MOV di, offset SBOX
oneSubByte:  mov al, [si]
             push ax              ; save original al
             GetHigherDigit
             mov bl, al           ; store higher nibble in bl
             pop ax               ; restore original al
             GetLowerDigit
             mov bh, al           ; store lower nibble in bh
             mov al, bl           ; move higher nibble back to al
             mov bl, 16
             mul bl               ; multiply higher nibble by 16
             add al, bh           ; add lower nibble
             mov bx, ax           ; bx = 16 * higher nibble + lower nibble
             mov al, [di+bx]
             mov [si], al
             INC si
             LOOP oneSubByte
